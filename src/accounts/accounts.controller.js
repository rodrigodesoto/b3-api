...
function register(req, res, next) {
    accountService.register(req.body, req.get('origin'), res)
        .then(() => res.json({ message: 'Registro bem-sucedido, verifique seu e-mail para obter instruções de verificação' }))
        .catch(next);
}
...
function validateResetToken(req, res, next) {
    accountService.validateResetToken(req.body)
        .then(() => res.json({ message: 'Token é valido' }))
        .catch(next);
}
...
function resetPassword(req, res, next) {
    accountService.resetPassword(req.body)
        .then(() => res.json({ message: 'Redefinição de senha bem-sucedida, agora você pode fazer login' }))
        .catch(next);
}
...
function createSchema(req, res, next) {
    const schema = Joi.object({
        ...
        password: Joi.string().min(6).required(),
        confirmPassword: Joi.string().valid(Joi.ref('password')).required(),
        role: Joi.string().valid(Role.Admin, Role.User, Role.System).required()
    });
    validateRequest(req, next, schema);
}
...
function updateSchema(req, res, next) {
    ...
    if (req.user.role === Role.Admin) {
        schemaRules.role = Joi.string().valid(Role.Admin, Role.User, Role.System).empty('');
    }
